...
class VariableLaneFDLink(CTMLink):
    ...
    def compute_demand_and_supplies(self, step):
        ...
        for cell in range(self.num_cells):
            overcritical = self.rho[cell, step]-self.fundamental_diagram.get_critical_density()
            max_demand = self.fundamental_diagram.get_capacity() * (1-self.alpha_d*overcritical/(self.fundamental_diagram.get_jam_density()-self.fundamental_diagram.get_critical_density()))*self.num_lanes
            if cell == 0:
