...
class TestMergeNode(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.node = MergeNode(
            node_id=1,
            outbound_link=self.outbound_link,
    ...
    def assert_for_case(self, capacities, demands, expected_flows):
        ...
        node = MergeNode(
            node_id=1, outbound_link=outbound_link, inbound_links=inbound_links
        )
        ...
        for i in range(3):
            self.assertAlmostEqual(
                inbound_links[i].set_outflow.call_args[0][0], expected_flows[i]
            )
        self.assertAlmostEqual(
            outbound_link.set_inflow.call_args[0][0], sum(expected_flows)
        )
