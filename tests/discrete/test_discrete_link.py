...
class TestLink(unittest.TestCase):
    ...
    def test_demand_increase(self):
        ...
        for t in range(self.link.T1):
            ...
            self.assertEqual(
                self.link.cumulative_inflows[t + 1]
                - self.link.cumulative_outflows[t + 1],
                len(self.link.vehicles),
            )

        self.link.compute_demand_and_supplies(self.link.T1)
        self.assertEqual(self.link.get_demand(), 1.0)
        self.assertEqual(
            self.link.cumulative_inflows[self.link.T1], self.link.T1 / 2 + 1
        )
        ...
        one_inflow = output_records[9]["cumulative_inflow"]-output_records[0]["cumulative_inflow"]
