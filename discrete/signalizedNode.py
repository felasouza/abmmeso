...
class SignalizedNode(DivergeNode):
    ...
    def compute_flows(self, t):
        ...
        right_of_ways = {
            link.link_id: self.signal_plan.get_right_of_way(link.link_id, t)
        ...
        priority_links = [
            link
            for link in self.inbound_links
            if right_of_ways[link.link_id] == SignalPlan.PROTECTED
        ]

        permitted_links = [
            link
            for link in self.inbound_links
            if right_of_ways[link.link_id] == SignalPlan.PERMITTED
        ]

        stop_permit_links = [
            link
            for link in self.inbound_links
            if right_of_ways[link.link_id] == SignalPlan.STOP_PERMIT
        ]

        self.current_flows = {link.link_id: 0 for link in self.inbound_links}
        self.inflow_order_by_outbound_link = {
            link.link_id: [] for link in self.outbound_links
        }

        self.remaining_supplies = [link.get_supply() for link in self.outbound_links]

        self.compute_for_links(
            priority_links, True
        )  # although not strictly necessary if the conflicting flows only affects the current step, but it is a good practice to avoid future errors
        ...
        for link in links_with_row:
            ...
            if not protected:
                ...
                for conflicting_link_id in self.conflicting_map[link.link_id]:
                    the_conflicting_link = [
                        link
                        for link in self.inbound_links
                        if link.link_id == conflicting_link_id
                    ][0]
                    if (
                        self.current_flows[conflicting_link_id] > 0
                        or the_conflicting_link.get_next_step_demand() > 0
                        or the_conflicting_link.get_flows_in_the_past_steps(
                            self.last_update, self.permitted_flow_steps
                        )
                        > 0
                    ):
            ...
            while demand >= 1:
                front_vehicle = link.get_vehicle_from_index(
                    self.current_flows[link.link_id]
                )
                ...
                if self.remaining_supplies[idx_outb] >= 1:
                    ...
                    demand -= 1
                    self.inflow_order_by_outbound_link[
                        self.outbound_links[idx_outb].link_id
                    ].append(front_vehicle)
                else:
                    break
